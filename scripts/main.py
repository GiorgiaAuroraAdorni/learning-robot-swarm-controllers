...
if __name__ == '__main__':
    ...
    g.check_dataset_conformity(runs_dir_omniscient, img_dir_omniscient, omniscient_controller, input=args.net_input)
    ...
    if args.plots_dataset:
        ...
        visualise_simulation(runs_dir_manual, img_dir_manual, 0,
                             'Distribution simulation %d - %s' % (0, manual_controller))
        visualise_simulation(runs_dir_manual, img_dir_manual, 1,
                             'Distribution simulation %d - %s' % (1, manual_controller))
        visualise_simulation(runs_dir_manual, img_dir_manual, 2,
                             'Distribution simulation %d - %s' % (2, manual_controller))
        visualise_simulation(runs_dir_manual, img_dir_manual, 3,
                             'Distribution simulation %d - %s' % (3, manual_controller))
        visualise_simulation(runs_dir_manual, img_dir_manual, 4,
                             'Distribution simulation %d - %s' % (4, manual_controller))
    ...
    g.check_dataset_conformity(runs_dir_manual, img_dir_manual, dataset_manual, input=args.net_input)
    ...
    if args.train_model and args.plots_net:
        print('\nTraining %s and generating plots…' % args.model)
    elif args.train_model and not args.plots_net:
        print('\nTraining %s…' % args.model)
    elif not args.train_model and args.plots_net:
    ...
    run_distributed(file, runs_dir_omniscient, model_dir, model_img, args.model, dataset_omniscient, dataset_manual,
                    train=args.train_model, generate_split=args.generate_split, plots=args.plots_net)
    ...
    if args.plots_dataset:
        ...
        visualise_simulation(runs_dir_learned, img_dir_learned, 0,
                             'Distribution simulation %d - %s' % (0, learned_controller))
        visualise_simulation(runs_dir_learned, img_dir_learned, 1,
                             'Distribution simulation %d - %s' % (1, learned_controller))
        visualise_simulation(runs_dir_learned, img_dir_learned, 2,
                             'Distribution simulation %d - %s' % (2, learned_controller))
        visualise_simulation(runs_dir_learned, img_dir_learned, 3,
                             'Distribution simulation %d - %s' % (3, learned_controller))
        visualise_simulation(runs_dir_learned, img_dir_learned, 4,
                             'Distribution simulation %d - %s' % (4, learned_controller))
    ...
    g.check_dataset_conformity(runs_dir_learned, img_dir_learned, dataset_learned, input=args.net_input)
